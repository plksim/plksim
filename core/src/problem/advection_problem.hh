...
template <int dim>
class AdvectionRhs : public Function<dim> {
public:
...
template <>
const Point<1> AdvectionRhs<1>::center_point = Point<1>(-0.75);

template <>
const Point<2> AdvectionRhs<2>::center_point = Point<2>(-0.75, -0.75);

template <>
const Point<3> AdvectionRhs<3>::center_point = Point<3>(-0.75, -0.75, -0.75);

template <int dim>
double AdvectionRhs<dim>::value(const Point<dim>& p, const unsigned int component) const {
  (void)component;
  Assert(component == 0, ExcIndexRange(component, 0, 1));
  const double diameter = 0.1;
  return ((p - center_point).norm_square() < diameter * diameter ? 0.1 / std::pow(diameter, dim)
                                                                 : 0.0);
}
...
template <int dim>
class AdvectionProblem : public ProblemBase {
public:
  AdvectionProblem();
...
private:
  struct AssemblyScratchData {
    ...
    AdvectionRhs<dim> right_hand_side;
...
public:
  template <int dim>
  static void estimate(const DoFHandler<dim>& dof, const Vector<double>& solution,
...
private:
  ...
  struct EstimateScratchData {
    EstimateScratchData(const FiniteElement<dim>& fe, const Vector<double>& solution,
                        Vector<float>& error_per_cell);
...
GradientEstimation::EstimateScratchData<dim>::EstimateScratchData(const FiniteElement<dim>& fe,
    ...
    : fe_midpoint_value(fe, QMidpoint<dim>(), update_values | update_quadrature_points),
      solution(solution), error_per_cell(error_per_cell), cell_midpoint_value(1),
...
GradientEstimation::EstimateScratchData<dim>::EstimateScratchData(
    ...
    : fe_midpoint_value(scratch_data.fe_midpoint_value.get_fe(),
      ...
      solution(scratch_data.solution), error_per_cell(scratch_data.error_per_cell),
      cell_midpoint_value(1), neighbor_midpoint_value(1) {
}

template <int dim>
void GradientEstimation::estimate(const DoFHandler<dim>& dof_handler,
                                  const Vector<double>& solution, Vector<float>& error_per_cell) {
...
void GradientEstimation::estimate_cell(const typename DoFHandler<dim>::active_cell_iterator& cell,
    ...
    if (!cell->at_boundary(face_n)) {
      ...
      if (neighbor->is_active())
        ...
        if (dim == 1) {
          ...
          Assert(neighbor_child->neighbor(face_n == 0 ? 1 : 0) == cell, ExcInternalError());

          scratch_data.active_neighbors.push_back(neighbor_child);
        } else
          for (unsigned int subface_n = 0; subface_n < face->n_children(); ++subface_n)
  ...
  scratch_data.fe_midpoint_value.get_function_values(scratch_data.solution,
                                                     scratch_data.cell_midpoint_value);
  ...
  for (const auto& neighbor : scratch_data.active_neighbors) {
    ...
    scratch_data.fe_midpoint_value.get_function_values(scratch_data.solution,
                                                       scratch_data.neighbor_midpoint_value);
    ...
    projected_gradient +=
        (scratch_data.neighbor_midpoint_value[0] - scratch_data.cell_midpoint_value[0]) / distance *
        y;
  }
...
template <int dim>
void AdvectionProblem<dim>::compute() {
...
template <int dim>
void AdvectionProblem<dim>::make_grid() {
...
void AdvectionProblem<dim>::assemble_system() {
  WorkStream::run(dof_handler.begin_active(), dof_handler.end(), *this,
                  &AdvectionProblem::local_assemble_system, &AdvectionProblem::copy_local_to_global,
...
AdvectionProblem<dim>::AssemblyScratchData::AssemblyScratchData(
    ...
    : fe_values(scratch_data.fe_values.get_fe(), scratch_data.fe_values.get_quadrature(),
                update_values | update_gradients | update_quadrature_points | update_JxW_values),
      fe_face_values(
          scratch_data.fe_face_values.get_fe(), scratch_data.fe_face_values.get_quadrature(),
...
void AdvectionProblem<dim>::local_assemble_system(
  ...
  scratch_data.right_hand_side.value_list(scratch_data.fe_values.get_quadrature_points(),
    ...
    if (face->at_boundary()) {
      ...
      for (unsigned int q_point = 0; q_point < n_face_q_points; ++q_point)
        if (scratch_data.fe_face_values.normal_vector(q_point) *
          ...
          for (unsigned int i = 0; i < dofs_per_cell; ++i) {
            for (unsigned int j = 0; j < dofs_per_cell; ++j)
              copy_data.cell_matrix(i, j) -= (scratch_data.face_advection_directions[q_point] *
            ...
            copy_data.cell_rhs(i) -= (scratch_data.face_advection_directions[q_point] *
                                      scratch_data.fe_face_values.normal_vector(q_point) *
...
void AdvectionProblem<dim>::copy_local_to_global(const AssemblyCopyData& copy_data) {
  hanging_node_constraints.distribute_local_to_global(copy_data.cell_matrix, copy_data.cell_rhs,
                                                      copy_data.local_dof_indices, system_matrix,
                                                      system_rhs);
}

template <int dim>
void AdvectionProblem<dim>::solve() {
  SolverControl solver_control(std::max<std::size_t>(1000, system_rhs.size() / 10),
...
void AdvectionProblem<dim>::refine_grid() {
  ...
  GridRefinement::refine_and_coarsen_fixed_number(triangulation, estimated_error_per_cell, 0.3,
                                                  0.03);
...
template <int dim>
void AdvectionProblem<dim>::output() const {
